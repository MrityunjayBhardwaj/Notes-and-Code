...
function meshGridRange(
  ...
  console.log(range);
  const a = tf
    .linspace(range.x.min - margin, range.x.max + margin, (typeof division === 'object')? division.x : division )
    .flatten()
    .arraySync();
  const b = tf
    .linspace(range.y.min - margin, range.y.max + margin, (typeof division === 'object')? division.y : division)
    .flatten()
    .arraySync();
...
function inputViz(divContainer, 
  ...
  // this.getSvgId = () =>{return }
   function init(){
    ...
    svgSettings.gridIntervel.x = svgSettings.gridIntervel.x || svgSettings.gridIntervel || 8;
    svgSettings.gridIntervel.y = svgSettings.gridIntervel.y || svgSettings.gridIntervel || 8;

    console.log(svgSettings.gridIntervel)
    // range of the plot
    const range = {
    ...
    let gridIntervelsX = tf.linspace(range.x.min, range.x.max, svgSettings.gridIntervel.x).flatten().arraySync();
    let gridIntervelsY = tf.linspace(range.y.min, range.y.max, svgSettings.gridIntervel.y).flatten().arraySync();
